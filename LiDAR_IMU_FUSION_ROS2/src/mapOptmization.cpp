...
//            int ret = pcl::io::savePCDFileBinary("/home/ztl/pointcloud2pcd/B.pcd", *globalMapCloud);
//            res->success = ret == 0;
...
//        };
    ...
    int saveMapCounter = 0;  // 全局计数器，用来跟踪保存的次数
    // 保存全局关键帧特征点集合
...
//      unused = system((std::string("mkdir -p ") + saveMapDirectory).c_str());
            ...
            //清空容器，释放内存
            const size_t maxSize = 100;  // 设置最大帧数为1000

            cout << "cornerCloudKeyFrames.size() = " << cornerCloudKeyFrames.size() << endl;

// 只清除超出 maxSize 的数据
            if (cornerCloudKeyFrames.size() > maxSize) {
                for (int i = 0; i < (cornerCloudKeyFrames.size() - maxSize); i++) {
                    cornerCloudKeyFrames[i]->clear();  // 清空最旧的角点云数据
                    surfCloudKeyFrames[i]->clear();    // 清空最旧的平面点云数据
                    cout << "Free memory for index " << i << "..." << endl;
                }
            }

//        // 对于平面点云
//          while (surfCloudKeyFrames.size() > maxSize) {
//              surfCloudKeyFrames.erase(surfCloudKeyFrames.begin());  // 删除最旧的平面点云
//              surfCloudKeyFrames.shrink_to_fit();  // 请求回收未使用的内存
//              cout << "Free memory for surfCloudKeyFrames..." << endl;
//          }
            saveMapCounter++;  // 每次调用时增加计数器
            if(req.resolution != 0)
            {
                stringstream cornerFilename, surfFilename;
                cornerFilename << saveMapDirectory << "/CornerMap_" << saveMapCounter << ".pcd";
                surfFilename << saveMapDirectory << "/SurfMap_" << saveMapCounter << ".pcd";
                pcl::io::savePCDFileBinary(cornerFilename.str(), *globalCornerCloud);
                pcl::io::savePCDFileBinary(surfFilename.str(), *globalSurfCloud);
            }
            else
            {
                stringstream cornerFilename, surfFilename;
            ...
            *globalMapCloud += *globalCornerCloud;
            *globalMapCloud += *globalSurfCloud;
            stringstream globalMapFilename;
            globalMapFilename << saveMapDirectory << "/GlobalMap_" << saveMapCounter << ".pcd";
            int ret = pcl::io::savePCDFileBinary(globalMapFilename.str(), *globalMapCloud);
            res.success = ret == 0;

            cout << "****************************************************" << endl;
            cout << "Saving map to pcd files completed (" << saveMapCounter << ")\n" << endl;
        }
        return true;
    }
